...
class OrderWorkflowStateMachine(RuleBasedStateMachine):
    ...
    @precondition(
        lambda self: self.order.status == OrderStatus.DRAFT and self.order.items.count() > 0
    )
    @rule()
    ...
    @precondition(
        lambda self: self.order.status != OrderStatus.DRAFT or self.order.items.count() == 0
    )
    @rule()
